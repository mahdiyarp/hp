...
router = APIRouter(prefix="/api/payments", tags=["payments"])  # auth enforced per-endpoint for test expectations
...
def list_payments(
    ...
    # Require Authorization header unless a dict override (tests override get_current_user with lambda returning dict)
...
def create_payment(payload: dict, session: Session = Depends(db.get_db)):
    ...
    if invoice_id:
        ...
        if not inv:
            raise HTTPException(status_code=400, detail="Invoice not found for payment")
        # Enforce overpay prevention: reject if cumulative would exceed invoice total
        total_paid = (
            session.query(models.Payment)
        ...
        if int(total_paid) + int(amount) > int(getattr(inv, 'total', 0) or 0):
            raise HTTPException(status_code=400, detail="Payment exceeds invoice total")
    obj = models.Payment(
...
def update_payment(payment_id: int, payload: schemas.PaymentCreate, session: Session = Depends(db.get_db), current=Depends(get_current_user)):
    ...
    session.commit()
    session.refresh(obj)
    # Adapted to unified log_activity signature
