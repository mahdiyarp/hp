...
def parse_invoice_file(path: str) -> Dict[str, Any]:
    ...
    # Heuristics: find party line (look for words like TO:, BUYER:, ط·آ·ط¢آ¸ط·آ¸ط¢آ¾ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط¢آ¸ط·آ«أ¢â‚¬آ ط·آ·ط¢آ·ط·آ¢ط¢آ´ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط¢آ ط·آ·ط¢آ·ط·آ¢ط¢آ¯ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط·إ’, ط·آ·ط¢آ·ط·آ¢ط¢آ®ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط·â€؛ط·آ¥أ¢â‚¬â„¢ط·آ·ط¢آ·ط·آ¢ط¢آ¯ط·آ·ط¢آ·ط·آ¢ط¢آ§ط·آ·ط¢آ·ط·آ¢ط¢آ±)
    party = None
    for ln in lines[:15]:
        if re.search(r'(?i)(to:|buyer:|seller:| ط·آ·ط¢آ¸ط·آ¸ط¢آ¾ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط¢آ¸ط·آ«أ¢â‚¬آ ط·آ·ط¢آ·ط·آ¢ط¢آ´ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط¢آ ط·آ·ط¢آ·ط·آ¢ط¢آ¯ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط·إ’| ط·آ·ط¢آ·ط·آ¢ط¢آ®ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط·â€؛ط·آ¥أ¢â‚¬â„¢ط·آ·ط¢آ·ط·آ¢ط¢آ¯ط·آ·ط¢آ·ط·آ¢ط¢آ§ط·آ·ط¢آ·ط·آ¢ط¢آ±|ط·آ·ط¢آ·ط·آ¢ط¢آ·ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط¢آ¸ط·آ¸ط¢آ¾)', ln):
            party = ln
            break
    ...
    amounts = []
    for ln in lines:
        for num in re.findall(r'\d[\d,\.]+', ln.replace('ط·آ·ط¢آ¸ط·آ¢ط¢آ¬', ',')):
            try:
                cleaned = num.replace(',', '').replace('ط·آ·ط¢آ¸ط·آ¢ط¢آ¬', '')
                val = float(cleaned)
    ...
    for ln in lines:
        ...
        if len(nums) >= 2:
            ...
            try:
                last = float(nums[-1].replace(',', '').replace('ط·آ·ط¢آ¸ط·آ¢ط¢آ¬', ''))
                prev = float(nums[-2].replace(',', '').replace('ط·آ·ط¢آ¸ط·آ¢ط¢آ¬', ''))
...
def parse_payment_file(path: str) -> Dict[str, Any]:
    ...
    date = draft.get('date')
    # Try to extract a reference (look for 'ref' or 'ط·آ·ط¢آ·ط·آ¢ط¢آ´ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط¢آ¦ط·آ·ط¢آ·ط·آ¢ط¢آ§ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط·إ’' keywords)
    ref = None
    txt = draft.get('raw_text_preview', '')
    m = re.search(r'(?i)(ref[:\s]*|reference[:\s]*|ط·آ·ط¢آ·ط·آ¢ط¢آ´ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط¢آ¦ط·آ·ط¢آ·ط·آ¢ط¢آ§ط·آ·ط¢آ·ط·آ¢ط¢آ±ط·آ·ط¢آ¸ط£آ¢أ¢â€ڑآ¬ط·إ’[:\s]*)([\w\-\d\/]+)', txt)
    if m:
        ref = m.group(2)
