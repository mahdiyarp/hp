...
export const PApi = {
  ...
  smsOtp: (code: string, mobile: string, template?: number) => fetchWithAuth('/api/apiir/otp/sms', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  startOtp: (mobile: string) => fetchWithAuth('/api/papi/otp/start', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mobile })
  }).then(async r => { if (!r.ok) throw new Error(await r.text()); return r.json(); }),
  verifyOtp: (mobile: string, code: string) => fetchWithAuth('/api/papi/otp/verify', {
