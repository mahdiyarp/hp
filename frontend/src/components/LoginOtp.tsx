        ...
        setTimeout(() => {
          try {
            ...
            const evt = new CustomEvent('api-error', {
              detail: { status: 200, message: 'ورود با OTP موفق بود' },
            })
  ...
  return (
    <div className="hp-card">
      ...
      <div className="mt-3 flex gap-2">
        <button className="hp-btn" onClick={sendSmsOtp}>
          ارسال کد پیامکی
        </button>
        <button className="hp-btn" onClick={callOtp}>
          OTP تلفنی
        </button>
        <button className="hp-btn" onClick={verify}>
          تأیید و ورود
        </button>
      </div>
      {status && <div className="mt-3 text-sm">نتیجه: {status}</div>}
    </div>
  )
}
