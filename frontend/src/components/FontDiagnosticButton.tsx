...
export default function FontDiagnosticButton() {
  async function runDiag() {
    try {
      ...
      const selectors = ['aside', 'h1', 'table']
      for (const s of selectors) {
        ...
        if (/\uFFFD/.test(txt)) results.corruption = true
      ...
      if (statusEl) {
        statusEl.textContent = `Fonts: ${fonts} | RTL: ${results.rtl ? 'OK' : 'MISSING'} | corruption: ${results.corruption}`
