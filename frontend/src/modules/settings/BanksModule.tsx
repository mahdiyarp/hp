...
export default function BanksModule() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const data = await apiGet<{ banks: Bank[]; branches: Branch[] }>(
  ...
  async function updateFromSources() {
    ...
    setError(null)
    try {
      const res = await apiPost<{ success: boolean; banks_count: number; branches_count: number }>(
        '\/api\/integrations\/iran-banks\/update',
        {},
      )
      await load()
  ...
  return (
    ...
    <section className={`${retroPanelPadded} space-y-4 min-h-[50vh]`}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-[#7a6b4f]">
            جستجو و بروزرسانی خودکار از منابع تنظیمات سیستم
          </div>
        </div>
        <div className="flex gap-2">
          <input
            ...
            placeholder="جستجو بانک/شعبه/شهر"
            className={`${retroInput} w-64`}
          />
          <button
            type="button"
          ...
          <button type="button" className={`${retroButton}`} onClick={load} disabled={loading}>
            بارگذاری
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          <div className="mb-2 text-sm text-[#7a6b4f]">
            {filteredBanks.length} بانک از {banks.length}
          </div>
        ...
        <div>
          <div className="mb-2 text-sm text-[#7a6b4f]">
            {filteredBranches.length} شعبه از {branches.length}
          </div>
          <table className="w-full border border-[#c5bca5] bg-[#faf4de] text-sm">
            <thead>
              <tr>
