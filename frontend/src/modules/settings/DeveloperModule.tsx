  ...
  async function askAssistant() {
    ...
    try {
      ...
      const res = await fetchWithAuth('/api/dev/assistant/run', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ topic: 'dev', details: aiInput }),
      })
  ...
  return (
    <section className={`${retroPanelPadded} space-y-4`}>
      <header>
        ...
        <p className="text-[11px] text-[#7a6b4f] mt-1">
          تاریخ مرجع: {smartDate.jalali ?? '—'} | {smartDate.isoDate ?? '—'}
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        <div className="border border-[#bfb69f] bg-[#f6f1df] px-4 py-3 shadow-inner space-y-2">
          ...
          <div className="flex gap-2">
            <button className="border px-3 py-1 text-xs" onClick={pingApi}>
              Ping /api/auth/me
            </button>
            <button className="border px-3 py-1 text-xs" onClick={fetchServerTime}>
              Get /api/time-sync
            </button>
          </div>
        ...
        <div className="border border-[#bfb69f] bg-[#f6f1df] px-4 py-3 shadow-inner space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <div className="flex items-end">
              <label className="text-xs text-[#7a6b4f] flex items-center gap-2">
                <input
                  type="checkbox"
          ...
          <div className="mt-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-2">
              <input
                ...
                value={otpTest.mobile}
                onChange={(e) => setOtpTest({ ...otpTest, mobile: e.target.value })}
              />
              <input
                className={`${retroInput}`}
            ...
            <button className={`${retroButton} mt-2`} disabled={testing} onClick={testOtpSend}>
              {testing ? 'در حال ارسال...' : 'ارسال کد تست'}
            </button>
          </div>
        </div>
        <div className="border border-[#bfb69f] bg-[#f6f1df] px-4 py-3 shadow-inner space-y-3">
          ...
          <div className="space-y-2">
            <div className="text-[11px] text-[#7a6b4f]">
              وضعیت: {assistantEnabled ? 'فعال' : 'غیرفعال'}
            </div>
            <textarea
            ...
            <div className="text-xs text-[#7a6b4f] whitespace-pre-wrap min-h-[4rem] border border-[#d7caa4] bg-[#fffaf0] p-2">
              {aiReply || 'پاسخ در اینجا نمایش داده می‌شود'}
            </div>
          </div>
        </div>
      </div>
