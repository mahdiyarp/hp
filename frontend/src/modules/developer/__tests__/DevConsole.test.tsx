  ...
  it('applies provider filter and triggers CSV export with BOM', async () => {
    mockFetchWithRoutes({
      '/api/sms/history?': (url: string) => {
        ...
        return { page: 1, limit: 50, total: 2, items: [
          { id: 1, provider, recipient: '+98912', message: 'A', status: 'sent', response_code: '200', response_message: 'ok', latency_ms: 100, tracking_code: 'TA', created_at: new Date().toISOString() },
          { id: 2, provider, recipient: '+98913', message: 'B', status: 'failed', response_code: '400', response_message: 'bad', latency_ms: 0, tracking_code: 'TB', created_at: new Date().toISOString() },
        ]};
      },
