...
export default function PApiPanel() {
  ...
  const [tab, setTab] = useState<
    'sms' | 'otp' | 'lines' | 'blacklist' | 'reports' | 'switches' | 'templates' | 'webhooks'
  >('sms')
    ...
    <div className="p-4 space-y-6 text-[#f3f2e6]">
      <div className="hp-card">
        <div className="flex gap-2 mb-3">
          {(
            ...
            <button
              key={t}
              className={`hp-btn ${tab === t ? '' : 'opacity-70'}`}
              onClick={() => setTab(t)}
            >
              {t === 'sms'
                ? 'ارسال پیامک'
        ...
        {tab === 'sms' && (
          <>
            ...
            />
            <div className="mt-3">
              <button className="hp-btn" onClick={sendSms}>
                ارسال
              </button>
            </div>
          </>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <select
                className="hp-input"
                value={otpTemplate ?? 1}
                onChange={(e) => setOtpTemplate(Number(e.target.value))}
              >
                <option value={1}>template 1</option>
                <option value={2}>template 2</option>
              </select>
              <div className="flex gap-2">
                <button className="hp-btn" onClick={startOtp}>
                  ارسال کد پیامکی
                </button>
                <button
                  className="hp-btn"
                ...
                <button className="hp-btn" onClick={verifyOtp}>
                  تأیید
                </button>
              </div>
            </div>
        ...
        {tab === 'lines' && (
          <div className="flex gap-2">
            <button className="hp-btn" onClick={loadLines}>
              بارگذاری خطوط
            </button>
          </div>
        )}
        {tab === 'blacklist' && (
          <div className="flex gap-2">
            <input
              ...
              onChange={(e) => setMobiles(e.target.value)}
            />
            <button className="hp-btn" onClick={() => addBlacklist(mobiles.trim())}>
              افزودن
            </button>
          </div>
        )}
        {tab === 'reports' && (
          <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
              ...
              />
              <button className="hp-btn" onClick={loadDailyReport}>
                گزارش روزانه
              </button>
              <button className="hp-btn" onClick={loadLines}>
                بارگذاری خطوط
        ...
        {tab === 'switches' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
              ...
              <button className="hp-btn" onClick={applyProvider}>
                اعمال
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center mt-3">
              ...
              />
              <button className="hp-btn" onClick={applyApiKey}>
                ثبت کلید
              </button>
            </div>
        ...
        {tab === 'templates' && (
          <div className="space-y-3">
            <div className="flex gap-2">
              <button className="hp-btn" onClick={listTemplates}>
                فهرست قالب‌ها
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              <button className="hp-btn" onClick={createTemplate}>
                ایجاد قالب
              </button>
            </div>
          </div>
        ...
        {tab === 'webhooks' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <button
                className="hp-btn"
                onClick={() => setWebhook('sms.status', 'https://example.com/webhook/sms')}
              >
                وبهوک وضعیت پیام
              </button>
              <button
                className="hp-btn"
                onClick={() => setWebhook('otp.verify', 'https://example.com/webhook/otp')}
              >
                وبهوک تأیید OTP
              </button>
            </div>
      ...
      <div className="hp-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input
            className="hp-input"
            placeholder="موبایل‌ها (با کاما یا فاصله)"
            value={mobiles}
            onChange={(e) => setMobiles(e.target.value)}
          />
          <input
            className="hp-input"
            placeholder="خط ارسال (اختیاری)"
            value={sender}
            onChange={(e) => setSender(e.target.value)}
          />
        </div>
        <textarea
          className="hp-input mt-3"
          rows={4}
          placeholder="متن شامل #var# مانند #name# و #code#"
          value={text}
          onChange={(e) => setText(e.target.value)}
        />
