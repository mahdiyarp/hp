...
export default function SmsPanel() {
  ...
  async function loadMetrics() {
    try {
      const res = await apiGet<{ days:number; points:Array<{day:string, ok:number, fail:number}> }>(`/api/sms/metrics/daily?days=14`)
  ...
  async function sendMany() {
    ...
    try {
      await persistBasic()
      const nums = recipients.split(/\s|,|;/).map(x=>x.trim()).filter(Boolean)
      ...
      for (const m of nums) {
        ...
        try {
          const r = await apiPost('/api/sms/send', payload)
      ...
      setResults(out)
      await loadHistory(); await loadMetrics()
    } finally { setSending(false) }
  }

  useEffect(()=>{ void (async()=>{ await loadConfig(); await loadLines(); await loadHistory(); await loadMetrics() })() }, [])
    ...
    <div className="space-y-6" dir="rtl">
      <section className={`${retroPanelPadded} grid grid-cols-1 md:grid-cols-3 gap-4`}>
        <div className={`${retroPanel} p-4 space-y-2`}>
          ...
          <select className="input w-full" value={provider} onChange={e=>setProvider(e.target.value as any)}>
            <option value="sms.ir">SMS.ir</option>
            <option value="ippanel">IPPanel</option>
          </select>
          <label className={retroMuted}>API Key</label>
          <input className="input w-full" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="x-api-key" />
          <label className={retroMuted}>انتخاب خط (اختیاری)</label>
          <select className="input w-full" value={selectedLine} onChange={e=>setSelectedLine(e.target.value)}>
        ...
        <div className={`${retroPanel} p-4 space-y-2`}>
          ...
          <textarea className="input w-full" rows={6} value={recipients} onChange={e=>setRecipients(e.target.value)} placeholder="0912..., با فاصله یا کاما جدا کنید" />
        </div>
        <div className={`${retroPanel} p-4 space-y-2`}>
          <p className={retroHeading}>متن پیام</p>
          <textarea className="input w-full" rows={6} value={message} onChange={e=>setMessage(e.target.value)} />
          <button className={retroButton} disabled={sending} onClick={sendMany}>{sending? 'در حال ارسال…' : 'ارسال'}</button>
        </div>
      </section>
      ...
      <section className={`${retroPanelPadded} grid grid-cols-1 md:grid-cols-2 gap-4`}>
        <div className={`${retroPanel} p-4`}>
          ...
          <pre className="text-xs whitespace-pre-wrap">{results.length ? JSON.stringify(results, null, 2) : '—'}</pre>
        </div>
        <div className={`${retroPanel} p-4`}>
          <p className={retroHeading}>متریک روزانه</p>
          <div className="text-xs">
            {metrics.map(m => (
              <div key={m.day} className="flex justify-between"><span>{m.day}</span><span>ok: {m.ok} • fail: {m.fail}</span></div>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className={`${retroPanelPadded}`}>
        ...
        <div className="overflow-auto">
          <table className="min-w-full text-sm">
            <thead><tr><th className="text-left">زمان</th><th>گیرنده</th><th>پیام</th><th>وضعیت</th><th>کد</th></tr></thead>
            <tbody>
              {history.map((h, i) => (
