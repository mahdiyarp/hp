// Developer tools for managing API keys
...
function DeveloperModule() {
  ...
  useEffect(() => {
    ...
    async function loadKeys() {
      ...
      try {
        ...
        const data = (await apiGet<ApiKey[]>('/api/developer/keys')) as ApiKey[]
        ...
        setError(err?.message || 'خطا در دریافت لیست کلیدها')
    ...
    async function createKey() {
      ...
      try {
        ...
        const data = (await apiPost<CreateKeyResponse>('/api/developer/keys', payload)) as CreateKeyResponse
        ...
        setError(err?.message || 'خطا در ایجاد کلید')
  ...
  async function rotate(keyId: number) {
    ...
    try {
      ...
      const data = await apiPost<RotateKeyResponse>(`/api/developer/keys/${keyId}/rotate`, {})
