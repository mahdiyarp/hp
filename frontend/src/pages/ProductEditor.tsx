    ...
    const load = async () => {
      if (mode === 'edit' && id) {
        try {
          ...
          setError(e?.message || 'خطا در دریافت کالا')
        }
      }
    }
    load()
  ...
  React.useEffect(() => {
    const t = setTimeout(async () => {
      ...
      try {
        setSaving(true)
        if (mode === 'create') {
          const created = await apiPost('/api/products', data)
          setData(created)
          setId(created?.id || null)
          if (created?.id) window.location.hash = `product-edit:${created.id}`
        } else if (mode === 'edit' && id) {
          const updated = await apiPut(`/api/products/${id}`, data)
          setData(updated)
        }
      } catch (e: any) {
    ...
    <div className="hp-card p-4" dir="rtl">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">
          {mode === 'create' ? 'ایجاد کالا' : 'ویرایش کالا'} {id ? `#${id}` : ''}
        </h2>
