...
const ContactEditor: React.FC<Props> = ({ mode }) => {
  ...
  React.useEffect(() => {
    ...
    if (parts[0] === 'contact-edit' && parts[1]) {
      ...
      if (Number.isFinite(cid)) setId(cid)
    }
  }, [])
    ...
    const load = async () => {
      if (mode === 'edit' && id) {
        try {
          ...
          setError(e?.message || 'خطا در دریافت مخاطب')
        }
      }
    }
    load()
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (mode === 'create') {
          ...
          if (created?.id) window.location.hash = `contact-edit:${created.id}`
        } else if (mode === 'edit' && id) {
          const updated = await apiPut(`/api/contacts/${id}`, data)
          setData(updated)
        }
      } catch (e: any) {
    ...
    <div className="hp-card p-4" dir="rtl">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">
          {mode === 'create' ? 'ایجاد مخاطب' : 'ویرایش مخاطب'} {id ? `#${id}` : ''}
        </h2>
                ...
                onClick={() => {
                  try {
                    localStorage.setItem(
                      'hp.task.prefill',
                      JSON.stringify({ entity_type: 'contact', entity_id: id }),
                    )
                  } catch {}
...
// Lightweight widgets
const ContactTimeline: React.FC<{ entityType: string; entityId: number }> = ({
  entityType,
  entityId,
}) => {
...
const RelatedTasks: React.FC<{ entityType: string; entityId: number }> = ({
  entityType,
  entityId,
}) => {
  const [items, setItems] = React.useState<any[]>([])
