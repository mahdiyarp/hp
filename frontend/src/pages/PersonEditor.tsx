...
const PersonEditor: React.FC<Props> = ({ mode }) => {
  ...
  const [data, setData] = React.useState<any>({ name: '', national_id: '', phone: '', email: '', address: '', status: 'active', tags: '' })
  ...
  React.useEffect(()=>{
    const load = async () => {
      if (mode === 'edit' && id) {
        try { const p = await apiGet(`/api/persons/${id}`); setData(p||{}); } catch(e:any){ setError(e?.message||'خطا در دریافت شخص') }
      }
    }
    load()
  }, [mode, id])
  ...
  React.useEffect(()=>{
    const t = setTimeout(async ()=>{
      ...
      try {
        setSaving(true)
        if (mode==='create') { const created = await apiPost('/api/persons', data); setData(created); setId(created?.id||null); if (created?.id) window.location.hash = `person-edit:${created.id}` }
        else if (mode==='edit' && id) { const updated = await apiPut(`/api/persons/${id}`, data); setData(updated) }
  ...
  return (
    <div className="hp-card p-4" dir="rtl">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">{mode==='create'?'ایجاد شخص':'ویرایش شخص'} {id?`#${id}`:''}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
        <div className="hp-card p-3">
          ...
          <input className="hp-input w-full mb-2" placeholder="کد ملی" value={data.national_id||''} onChange={e=>setField('national_id', e.target.value)} />
          ...
          <textarea className="hp-input w-full" rows={3} placeholder="برچسب‌ها (با کاما)" value={data.tags||''} onChange={e=>setField('tags', e.target.value)} />
        </div>
        <div className="hp-card p-3">
...
const EntityTimeline: React.FC<{entityType: string; entityId: number}> = ({entityType, entityId}) => {
...
const EntityTasks: React.FC<{entityType: string; entityId: number}> = ({entityType, entityId}) => {
  const [items, setItems] = React.useState<any[]>([])
  const [loading, setLoading] = React.useState(false)
