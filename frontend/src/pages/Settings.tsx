...
const Settings: React.FC = () => {
  ...
  const save = async () => {
    ...
    setMsg(null)
    try {
      await apiPut('/api/settings', flatten(data))
      setMsg('ذخیره شد')
    } catch (e: any) {
  ...
  return (
    <div className="hp-card p-4" dir="rtl">
      ...
      <div className="mt-3 text-sm text-[var(--primary)]/75">
        {loading
          ? 'در حال بارگذاری...'
          : 'پیکربندی برنامه و دسترسی‌ها را می‌توانید از این بخش مدیریت کنید.'}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
        <div className="hp-card p-3">
          ...
          <input
            type="checkbox"
            checked={!!data.rtl}
            onChange={(e) => setField('rtl', e.target.checked)}
          />
        </div>
        <div className="hp-card p-3">
          ...
          <input
            className="hp-input w-full mb-2"
            type="number"
            value={data.invoices?.default_tax || 0}
            onChange={(e) => setField('invoices.default_tax', Number(e.target.value))}
          />
          <label className="block text-xs mb-1">تخفیف پیش‌فرض (%)</label>
          <input
            className="hp-input w-full mb-2"
            type="number"
            value={data.invoices?.default_discount || 0}
            onChange={(e) => setField('invoices.default_discount', Number(e.target.value))}
          />
          <label className="block text-xs mb-1">شماره‌گذاری خودکار</label>
          <input
            type="checkbox"
            checked={!!data.invoices?.auto_numbering}
            onChange={(e) => setField('invoices.auto_numbering', e.target.checked)}
          />
        </div>
        <div className="hp-card p-3">
          ...
          <input
            type="checkbox"
            checked={!!data.contacts?.duplicate_check}
            onChange={(e) => setField('contacts.duplicate_check', e.target.checked)}
          />
        </div>
        <div className="hp-card p-3">
          ...
          <input
            type="checkbox"
            checked={!!data.security?.otp_enabled}
            onChange={(e) => setField('security.otp_enabled', e.target.checked)}
          />
          <label className="block text-xs mb-1 mt-2">اجازه دسترسی مهمان</label>
          <input
            type="checkbox"
            checked={!!data.security?.allow_guest}
            onChange={(e) => setField('security.allow_guest', e.target.checked)}
          />
        </div>
      </div>
    </div>
