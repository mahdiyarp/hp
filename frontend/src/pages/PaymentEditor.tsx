...
export default function PaymentEditor() {
  ...
  const [data, setData] = useState<PaymentIn>({
    direction: 'in',
    amount: 0,
    method: 'cash',
    status: 'draft',
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded shadow w-full max-w-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="text-sm">
            ...
            <input
              ...
              onChange={(e) =>
                setData({ ...data, invoice_id: parseInt(e.target.value || '0') || undefined })
              }
            />
          </label>
          <label className="text-sm">
            ...
            <input
              ...
              type="number"
              value={data.amount}
              onChange={(e) => setData({ ...data, amount: parseInt(e.target.value || '0') || 0 })}
            />
          </label>
          <label className="text-sm">
            جهت
        ...
        <div className="flex items-center justify-end space-x-2 rtl:space-x-reverse">
          <button className="px-3 py-2 bg-slate-200 rounded" onClick={close}>
            انصراف
          </button>
          <button
