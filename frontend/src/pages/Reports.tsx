...
const Reports: React.FC = () => {
  const [filters, setFilters] = React.useState<any>({ from: '', to: '', customer: '', product: '', status: '' })
  ...
  const canView = React.useMemo(()=>{
    try { const raw = localStorage.getItem('hp.perms'); if (!raw) return true; const perms = JSON.parse(raw); return !!perms?.reports_view } catch { return true }
  }, [])
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(arr)
    } catch(e:any) { setError(e?.message||'خطا در دریافت گزارش'); setItems([]) } finally { setLoading(false) }
  }

  React.useEffect(()=>{ load() }, [filters, page, limit])
  ...
  return (
    <div className="hp-card p-4" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mt-3">
        <input className="hp-input" placeholder="از تاریخ (YYYY-MM-DD)" value={filters.from} onChange={e=>setFilters({...filters, from: e.target.value})} />
          ...
          <tbody>
            {items.map((r, idx)=>(
              <tr key={idx} className="border-t">
                ...
                <td className="px-3 py-2">{new Intl.NumberFormat('fa-IR').format(Number(r.total || r.amount || 0))}</td>
                <td className="px-3 py-2"><span className={`hp-badge ${r.status==='final'?'success':r.status==='paid'?'success':'dark'}`}>{r.status||'-'}</span></td>
              </tr>
            ))}
            {items.length===0 && (
              <tr>
                <td className="px-3 py-6 text-center text-[var(--primary)]/70" colSpan={4}>{loading?'در حال بارگذاری...':'موردی یافت نشد'}</td>
              </tr>
            )}
          </tbody>
        </table>
